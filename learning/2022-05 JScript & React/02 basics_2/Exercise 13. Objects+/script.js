...
function calculateTheArea(object) {
    let shopsArea = 0;
    for (let i = 0; i < object.shops.length; i++) {
        shopsArea += object.shops[i].width * object.shops[i].length;
    }
    return shopsArea;
}
function calculateTheVolume(object) {
    let shopsVolume = calculateTheArea(object);
    shopsVolume *= object.height;
    return shopsVolume;
}
function isBudgetEnough(object) {
    let result = calculateTheVolume(object);
    result *= object.moneyPer1m3;
    if (result < object.budget) {
        result = 'Budget is enough';
    } else {
        result = 'Budget is NOT enough';
    }
    return result;
}

console.log(calculateTheArea(shoppingMallData));
console.log(calculateTheVolume(shoppingMallData));
